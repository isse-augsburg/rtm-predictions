...
def plot_sensorgrid(filename):
    ...
    raw_coords_as_np_array = f[
        'post/constant/entityresults/NODE/'
        'COORDINATE/ZONE1_set0/erfblock/res'][()]
...
def get_filelist_within_folder(root_directory):
    ...
    for (dirpath, _, filenames) in walk(root_directory):
        if filenames:
            filenames = [dirpath + '/' + f for f in filenames if
                         f.endswith('.erfh5')]
            dataset_filenames.extend(filenames)
    return dataset_filenames
...
def get_all_sequences_for_file(filename, t_begin, t_end, t_delta,
                               t_target_offset, t_sequence_distance, t_final):
    all_sequences = list()

    try:
        while t_end + t_target_offset < t_final:
            instance = get_fillings_at_times(filename, t_begin, t_end, t_delta,
...
def get_all_sensor_values(filename):
    ...
    pressure_array = \
        f['post']['multistate']['TIMESERIES1']['multientityresults']['SENSOR'][
            'PRESSURE']['ZONE1_set1']['erfblock'][
            'res'][
            ()]
    ...
    try:
        last_filling_factor = \
            f['post']['singlestate'][state]['entityresults']['NODE'][
                ...
                ()]
        label = f['post']['singlestate'][state]['entityresults']['NODE'][
            'FILLING_FACTOR']['ZONE1_set1']['erfblock'][
            'indexval'][()]
        last_filling_factor = last_filling_factor.flatten()
        non_zeros = np.count_nonzero(last_filling_factor)
...
if __name__ == "__main__":
    filename = '/run/user/1001/gvfs/smb-share:server=137.250.170.56,' \
               'share=share/data/RTM/Leoben/output/with_shapes/' \
               '2019-07-23_15-38-08_5000p/39/2019-07-23_15-38-08_39_RESULT' \
               '.erfh5'
