...
def with_matplotlib():
    ...
    fvc = f[
        'post/constant/entityresults/SHELL/FIBER_FRACTION/ZONE1_set1/erfblock/res'].value
    ...
    for state in states:
        ...
        filling = \
            f['post']['singlestate'][state]['entityresults']['NODE'][
                ...
                ()]
        mean_of_neighbour_nodes = filling[triangle_coords].reshape(
            len(triangle_coords), 3).mean(axis=1)
        tpc = ax.tripcolor(x, y, triangle_coords,
                           facecolors=mean_of_neighbour_nodes,
                           edgecolors='none')
        # plt.show()
        ax.set_axis_off()
...
def with_pil():
    ...
    fvc = f[
        'post/constant/entityresults/SHELL/FIBER_FRACTION/ZONE1_set1/erfblock/res'].value
    perm_k1 = f[
        'post/constant/entityresults/SHELL/PERMEABILITY1/ZONE1_set1/erfblock/res'].value
    perm_k2 = f[
        'post/constant/entityresults/SHELL/PERMEABILITY2/ZONE1_set1/erfblock/res'].value
    thickness = f[
        'post/constant/entityresults/SHELL/THICKNESS/ZONE1_set1/erfblock/res'].value
    ...
    # exit()
    with Pool() as p:
        p.map(partial(plot_wrapper, triangle_coords, scaled_coords, fillings),
              range(len(fillings)))


def plot_wrapper(triangle_coords, scaled_coords, fillings, index):
    filling = fillings[index]
    means_of_neighbour_nodes = filling[triangle_coords].reshape(
        len(triangle_coords), 3).mean(axis=1)
    draw_polygon_map(index, means_of_neighbour_nodes, scaled_coords,
                     triangle_coords, colored=False)


def draw_polygon_map(index, values_for_triangles, scaled_coords,
                     triangle_coords, colored=True):
    ...
    for i in range(len(triangle_coords)):
        ...
        if not colored:
            draw.polygon(scaled_coords[triangle_coords[i]],
                         fill=(int(val * 255)))
        else:
            if val == 0.0:
                draw.polygon(scaled_coords[triangle_coords[i]],
                ...
                h = 3.6 * val
                col = tuple(
                    int(round(i * 255)) for i in colorsys.hsv_to_rgb(h, 1, 1))
