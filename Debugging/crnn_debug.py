...
from Pipeline import erfh5_pipeline as pipeline, data_gather as dg, \
    data_loader_sensor as dls

# path = ['/run/user/1001/gvfs/smb-share:server=137.250.170.56,
...
# model_path = '/run/user/1001/gvfs/smb-share:server=137.250.170.56,
# share=home/l/o/lodesluk/models/crnn_weird.pt'
...
def create_dataGenerator_pressure_sequence():
    try:
        generator = pipeline.ERFH5DataGenerator(
            ...
            batch_size=1, epochs=1, max_queue_length=256,
            num_validation_samples=2)
        """ generator = pipeline.ERFH5_DataGenerator(
        path, data_processing_function = dl.get_all_sensor_sequences, 
        data_gather_function = dl.get_filelist_within_folder,
            batch_size=batchsize, epochs=epochs ,max_queue_length=4096, 
...
if __name__ == "__main__":
    ...
    """ class MyModel(nn.Module):
        def __init__(self):
            ...
            image_modules = list(model.cnn.children())[:-1] 
