...
def create_images_for_file(filename,
                           main_folder="/run/user/1002/gvfs/smb-share:"
                                       "server=137.250.170.56,"
                                       "share=home/s/c/schroeni/"
                                       "Data/Images_Debug/"):
    ...
    coord_as_np_array = f[
        'post/constant/entityresults/NODE/COORDINATE/'
    ...
    for i, state in tqdm(enumerate(all_states), total=np.shape(all_states)[0]):
        try:
            ...
            f_name = "{:05d}".format(i) + "_" + '{:1.6f}'.format(
                filling_percentage) + "_" + '{:010.6f}'.format(time)
            # print(f_name)
            create_img(norm_coords=_coords, data=filling,
                       folder=main_folder + folder_name, filename=f_name)
            # create_np_image(norm_coords=_coords, data=filling,
            # folder=main_folder+folder_name, filename=f_name)
...
def create_img(target_shape=(150, 150), norm_coords=None, data=None, folder="",
               filename=""):
    if norm_coords is None or \
            data is None or \
            folder is None or \
            filename is None:
        print("ERROR")
    ...
    for i, value in enumerate(data):
        coord = norm_coords[i]
        x, y = int(round(coord[0] * (target_shape[0] - 1))), int(
            round(coord[1] * (target_shape[1] - 1)))
        # print(x,y)
...
if __name__ == "__main__":
    create_images_for_file(
        ...
        "2019-04-23_10-23-20_200p/91/2019-04-23_10-23-20_91_RESULT.erfh5")
